...
export default function Home() {
  ...
  const loadGlobalReport = async (id: string) => {
    try {
      ...
      setLanguage(data.language as Language || 'zh-TW'); // fallback
      setActiveTab(Dimension.FINAL);
      setError(null);
      setCurrentReportId(data.id); // Enable sharing
      setIsProcessing(false);
      setIsMenuOpen(false);
            ...
            {historyTab === 'global' && (
              globalHistory.length === 0 ? (
                ...
                globalHistory.map((item) => (
                  <div key={item.id} className="p-3 mb-2 rounded-lg bg-white/5 hover:bg-white/10 transition cursor-pointer" onClick={() => loadGlobalReport(item.id)}>
